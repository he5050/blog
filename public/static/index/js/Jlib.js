...
*
*
* cookie操纵函数方法，类似jquery.cookie => J.cookie(key,[value],[{expires: 365, path: '/', domain: 'xxxx',secure: false}]) 
...
* ========> 设置cookie时options参数为json对象格式 格式范例 => {expires: 365, path: '/', domain: 'xxxxx',secure: false}
...
(function(window, undefined) {
    ...
    //加载js
	J.aysnc_load_js=function (url) {
		if (window.attachEvent) {
			window.attachEvent('onload', function() {J.async_load(url)}); 
		}else{
			window.addEventListener('load', function() {J.async_load(url)}, false); 
            }
	}
    //执行
    J.async_do_func=function (func) {
		if (window.attachEvent) 
    ...
    J.Cookie=function(key,value,options) {
        if(options == undefined) { //无options选项，options选项中 过期时间以天为单位
            options = {expires: 365, path: '/',secure: false};//默认cookie设置{expires: 365, path: '/', domain: 'example.com',secure: true}
        }
        /*内部方法--cookie是否存在*/
